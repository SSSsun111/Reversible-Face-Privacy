...
class PULSE(torch.nn.Module):
    def __init__(self, cache_dir, verbose=True):
        ...
        # Convert cache_dir to Path object and ensure it exists
        cache_dir = Path(cache_dir)
        cache_dir.mkdir(parents=True, exist_ok=True)

        # Define model file paths
        synthesis_path = cache_dir / "synthesis.pt"
        mapping_path = cache_dir / "mapping.pt"

        # Check if synthesis model exists and load it
        if self.verbose: print("Loading Synthesis Network")
        if synthesis_path.exists():
            if self.verbose: print(f"Using local synthesis model file: {synthesis_path}")
            self.synthesis.load_state_dict(torch.load(synthesis_path))
        else:
            if self.verbose: print(f"Local synthesis model not found. Downloading...")
        ...
        # Freeze synthesis parameters
        for param in self.synthesis.parameters():
            param.requires_grad = False

        self.lrelu = torch.nn.LeakyReLU(negative_slope=0.2)

        # Check if the gaussian fit parameters already exist
        gaussian_fit_path = Path("gaussian_fit.pt")
        if gaussian_fit_path.exists():
            self.gaussian_fit = torch.load(gaussian_fit_path)
        else:
            if self.verbose: print("\tLoading Mapping Network")
            mapping = G_mapping().cuda()

            # Check if mapping model exists locally
            if mapping_path.exists():
                if self.verbose: print(f"Using local mapping model file: {mapping_path}")
                mapping.load_state_dict(torch.load(mapping_path))
            else:
                if self.verbose: print(f"Local mapping model not found. Downloading...")
                with open_url("https://drive.google.com/uc?id=14R6iHGf5iuVx3DMNsACAl7eBr7Vdpd0k", cache_dir=cache_dir,
                              verbose=verbose) as f:
                    mapping.load_state_dict(torch.load(f))
                # Save the mapping model locally
                torch.save(mapping.state_dict(), mapping_path)
            ...
            with torch.no_grad():
                ...
                if self.verbose: print(f"\tSaved \"{gaussian_fit_path}\"")
    ...
    def forward(self, ref_im,
        ...
        print(f"Final L2 loss: {min_l2}, eps threshold: {eps}")
        if (min_l2 <= eps):
            print("Found solution within epsilon threshold!")
        else:
            print("Could not find a face that downscales correctly within epsilon")
            print("Outputting best result anyway...")
