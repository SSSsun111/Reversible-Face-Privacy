...
from datasets import *  # 确保这里能导入我们新创建的paired_test_loader
import config
...
if args.use_model == 'StegFormer-B':
    encoder = StegFormer(1024, input_dim=(args.num_secret + 1) * 3, cnn_emb_dim=16, output_dim=3,
                         drop_key=False, patch_size=2, window_size=8, output_act=None,
...
if args.use_model == 'StegFormer-L':
    encoder = StegFormer(1024, input_dim=(args.num_secret + 1) * 3, cnn_emb_dim=32, output_dim=3,
                         depth=[2, 2, 2, 2, 2, 2, 2, 2, 2])
    decoder = StegFormer(1024, input_dim=3, cnn_emb_dim=32, output_dim=args.num_secret * 3,
                         depth=[2, 2, 2, 2, 2, 2, 2, 2, 2])
...
for j in range(1):
    with torch.no_grad():
        ...
        # 修改：使用paired_test_loader而不是分别的loader
        for cover, secret, filename in paired_test_loader:
            ...
            # 使用filename确保文件名匹配
            base_filename = os.path.splitext(filename[0])[0]  # 获取基本文件名（无扩展名）

            # 保存图像
            torchvision.utils.save_image(cover, args.path + '/image/wo_clamp/cover/' + f'{base_filename}.png')
            torchvision.utils.save_image(secret, args.path + '/image/wo_clamp/secret/' + f'{base_filename}.png')
            torchvision.utils.save_image(encode_img, args.path + '/image/wo_clamp/stego/' + f'{base_filename}.png')
            torchvision.utils.save_image(decode_img, args.path + '/image/wo_clamp/secret_rev/' + f'{base_filename}.png')

            i += 1  # 下一张图像
            print(f"img {i} ({base_filename}):")
            print(
                "PSNR_Y_cover:" + str(np.mean(psnry_encode_temp)) + " PSNR_Y_secret:" + str(np.mean(psnry_decode_temp)))
            print("SSIM_cover:" + str(np.mean(ssim_encode)) + " SSIM_secret:" + str(np.mean(ssim_decode)))
...
for j in range(1):
    ...
    with torch.no_grad():
        ...
        for cover, secret, filename in paired_test_loader:
            ...
            # 保存图像
            torchvision.utils.save_image(cover, args.path + '/image/clamp/cover/' + f'{base_filename}.png')
            torchvision.utils.save_image(secret, args.path + '/image/clamp/secret/' + f'{base_filename}.png')
            torchvision.utils.save_image(encode_img, args.path + '/image/clamp/stego/' + f'{base_filename}.png')
            torchvision.utils.save_image(decode_img, args.path + '/image/clamp/secret_rev/' + f'{base_filename}.png')

            i += 1  # 下一张图像
            # 如果需要打印每个图像的指标，可以取消下面的注释
            # print(f"img {i} ({base_filename}):")
